...
(()=>{
  ...
  localStorage.setItem("astroImageNowSettings",JSON.stringify({
    ...
    locations:[
      {id:"home",name:"Home",lat:44.98028,lon:-85.21117,bortle:5,alertThreshold:"green"},
      {id:"jgap",name:"JGAP",lat:45.1,lon:-85.4,bortle:3,alertThreshold:"yellow"}
    ]
  }));

  const astroRows=Array.from({length:168},(_,index)=>({
  ...
  window.fetch=(input,options={})=>{
    ...
    if(url.includes("/GetForecastData")){
      ...
      return json({TimeZone:"America/Detroit",ModelTime:hourIso(0),APICreditsRemaining:742,HourlyForecast:rows});
    }
