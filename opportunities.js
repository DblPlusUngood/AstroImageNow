...
const ImagingOpportunities=(()=>{
  ...
  function prepare(snapshot,summarizeWeather,now=Date.now()){
    ...
    return{
      ...
      astronomy:(snapshot?.forecast?.HourlyForecast||[]).map(row=>({ms:Date.parse(row.UTCForecastHour),row})).filter(r=>Number.isFinite(r.ms)).sort((a,b)=>a.ms-b.ms),
  ...
  function assess(ms,data,rig,mode){
    ...
    const needs=mode==="planetary"?["Cloud","Seeing","Wind"]:["Cloud","Transparency","Wind",...(rig.focalLengthMm>=1000?["Seeing"]:[])];
    ...
    if(mode==="planetary"){
      ...
      if(Math.min(...astro.map(r=>r.row.Seeing.ActualValue))<minimum)return{kind:"limited",reason:`Seeing below ${minimum}/5 for this configuration`};
    }else{
      if(worst("Transparency")>13)return{kind:"limited",reason:"Transparency is limiting"};
      if(rig.focalLengthMm>=1000&&Math.min(...astro.map(r=>r.row.Seeing.ActualValue))<3)return{kind:"limited",reason:"Seeing below 3/5 for long focal length"};
    }
    const watch=summaries.find(s=>s.severity==="watch");
    if(watch)return{kind:"caution",reason:watch.watch};
    ...
    if(dew.some(value=>value===null))return{kind:"caution",reason:"Dew margin unavailable"};
    if(Math.min(...dew)<4)return{kind:"caution",reason:"Narrow dew margin; plan dew control"};
  ...
  function match(result,data,rig,mode,filter=null){
    const samples=result.samples.map(s=>{
      ...
      if(mode!=="planetary"&&assessment.kind==="supported"&&P.moonCaution(result.target,s,filter))assessment={kind:"caution",reason:"Moonlight caution"};
      return{...s,assessment};
    });
    ...
    const caution=P.longestWindow(samples,s=>eligible(s)&&["supported","caution"].includes(s.assessment.kind));
    const window=supported||caution;
    ...
    const kind=supported?"supported":caution?"caution":!result.geometryWindow?"geometry":data.stale?"stale":reasons.some(r=>/coverage|incomplete/.test(r))?"unknown":"limited";
    const label={supported:"Forecast-supported window",caution:"Window with weather caution",geometry:"No usable altitude window",stale:"Stale forecast · geometry only",unknown:"Forecast coverage incomplete",limited:"No matching forecast window"}[kind];
    return{kind,label,window,reasons,samples};
  }
